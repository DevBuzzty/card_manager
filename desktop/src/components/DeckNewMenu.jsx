...
export default function DeckNewMenu({ onEmpty, onYdkFile, onPaste }) {
  ...
  return (
    <div className="relative">
      <button onClick={() => setOpen((v) => !v)} className="p-1.5 bg-accent hover:brightness-110 text-accent-fg rounded transition-colors" title="Neues Deck">
        <Plus className="w-4 h-4" />
      </button>
      {open && (
        <div className="absolute right-0 top-full mt-1 z-20 w-52 bg-bg border border-line rounded-lg shadow-sm py-1">
