...
const Row = ({ label, children, className = 'text-ink' }) => (
  <div className="flex justify-between"><span className="text-ink-muted">{label}</span><span className={`font-mono ${className}`}>{children}</span></div>
);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80" onClick={onClose}>
      <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto bg-obsidian-700 border border-line rounded-2xl p-5 space-y-4" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-bold text-ink">Verkauf</h2>
        ...
        {!sale ? <p className="text-ink-faint">{error || '…'}</p> : (
          <>
            {!form && (
              <>
                ...
                {orphaned && <p className="text-sm text-crit">Position ohne verkauftes Exemplar – bitte prüfen. Sie zählt in der Übersicht nicht mit.</p>}
                <div className="text-sm space-y-0.5 bg-obsidian-800 border border-line rounded-lg p-3">
                  ...
                  <Row label="Differenz" className={net >= market ? 'text-emerald-400' : 'text-crit'}>{diffText(net, market)}</Row>
                </div>
              </>
            )}
            ...
            {form && (
              <div className="space-y-3">
                ...
                <div className="text-sm">
                  <div className="text-ink">Netto {euroCentsText(formNet)}</div>
                  <div className={formNet >= formMarket ? 'text-emerald-400' : 'text-crit'}>{diffText(formNet, formMarket)} gegenüber Marktwert</div>
                </div>
              </div>
            )}
              ...
              {items.map((it) => {
                ...
                return (
                  <div key={it.copy_id} className={`flex items-center gap-3 p-2 rounded-lg border border-line ${gone ? 'opacity-50' : ''}`}>
                    {it.image_url ? <img src={it.image_url} alt="" className="w-10 h-14 object-cover rounded" /> : <div className="w-10 h-14 rounded bg-obsidian-800" />}
                    <div className="flex-1 min-w-0 text-sm">
                      ...
                      <div className="text-[11px] font-mono text-ink-muted">{it.condition} · {it.edition}</div>
                    </div>
                    <div className="text-right text-[11px] font-mono">
                      <div className="text-ink-muted">Marktwert {euroCentsText(toCents(it.value_at_sale) || 0)}</div>
                      {gone ? <div className="text-ink-faint">zurückgenommen</div> : <div className="text-gold">Anteil {euroCentsText(toCents(it.share) || 0)}</div>}
                    </div>
                    {form && !gone && (
                ...
                {form ? (
                  ...
                  <>
                    <button type="button" onClick={cancel} disabled={busy}
                      className="px-3 py-2 rounded-lg text-sm border border-crit/40 text-crit hover:bg-crit/10 disabled:opacity-50">Stornieren</button>
                    <button type="button" onClick={startEdit} disabled={busy}
                      className="px-4 py-2 rounded-lg text-sm bg-space-violet text-white disabled:opacity-50">Bearbeiten</button>
                  </>
                )}
              </div>
            )}
