...
export default function SealedList() {
  ...
  const scanHint = (
    <span className="inline-flex items-center gap-1 text-xs text-good">
      Geöffnet — <Link to={ROUTES.scannen} className="underline hover:text-text">Jetzt scannen</Link>
      <button type="button" onClick={() => setOpened(null)} aria-label="Hinweis schließen" className="text-muted hover:text-text">
        <X className="w-3 h-3" />
      </button>
    </span>
  );
  ...
  return (
    <div className="h-full overflow-auto">
      ...
      {writeError && <p className="text-sm text-bad mb-3">{writeError}</p>}
      {state.error && <p className="text-sm text-bad mb-3">{state.error}</p>}
      {/* Die Zeile ist nach dem Öffnen des letzten Exemplars weg — der Hinweis steht dann hier. */}
      {opened?.deleted && <p className="text-sm text-text mb-3">„{opened.name}“ · {scanHint}</p>}
      ...
      {data && data.items.length === 0 && (
        <div className="text-center text-muted py-10">Noch kein Sealed-Bestand</div>
      )}
      {data && data.items.length > 0 && (
              ...
              <div key={item.sealed_id} className="flex flex-wrap items-center gap-4 px-4 py-3">
                ...
                <div className="flex items-center gap-1">
                  <button type="button" onClick={() => minus(item)} disabled={rowBusy} title="Menge verringern"
                          className="p-1.5 rounded-lg border border-line text-muted hover:text-text disabled:opacity-50 disabled:cursor-not-allowed"><Minus className="w-3.5 h-3.5" /></button>
                  <span className="w-8 text-center font-mono text-sm text-text">{item.quantity}</span>
                  <button type="button" onClick={() => plus(item)} disabled={rowBusy} title="Menge erhöhen"
                          className="p-1.5 rounded-lg border border-line text-muted hover:text-text disabled:opacity-50 disabled:cursor-not-allowed"><Plus className="w-3.5 h-3.5" /></button>
                </div>
                ...
                <button type="button" onClick={() => open(item)} disabled={rowBusy}
                        className="flex items-center gap-1 text-xs text-muted hover:text-text border border-line rounded-lg px-2 py-1 disabled:opacity-50 disabled:cursor-not-allowed">
                  <PackageOpen className="w-3.5 h-3.5" /> Geöffnet
                </button>
                <button type="button" onClick={() => remove(item)} disabled={rowBusy} title="Löschen"
                        className="p-1.5 rounded-lg text-bad hover:bg-bad/10 disabled:opacity-50 disabled:cursor-not-allowed"><Trash2 className="w-4 h-4" /></button>
              </div>
            );
          })}
        </div>
