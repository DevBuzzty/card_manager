...
export default function ListingPhotos({ listingId, onChanged }) {
  ...
  const btn = 'px-2 py-1 rounded-lg text-xs bg-obsidian-600 border border-line text-ink hover:border-space-violet/40 disabled:opacity-50';

  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <label className="text-xs text-ink-muted uppercase tracking-wider">Eigene Fotos {photos && `(${photoCountText(photos.length)})`}</label>
        <button type="button" disabled={busy || !photos || photos.length >= MAX_OWN_PHOTOS} onClick={add} className={btn}>Fotos hinzufügen</button>
      </div>
      {photos === null ? <p className="text-sm text-ink-faint">…</p> : (
          ...
          {photos.map((p, i) => (
            <div key={p.photo_id} className="relative w-16 h-16 shrink-0">
              ...
              <div className="absolute inset-x-0 bottom-0 flex justify-between px-0.5 pb-0.5">
                ...
                <button type="button" disabled={busy} onClick={() => remove(p.photo_id)} className="text-[10px] px-1 rounded bg-black/60 text-crit">✕</button>
