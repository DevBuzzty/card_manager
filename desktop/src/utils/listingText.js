// Spec H3a -- ZWILLING (wortgleich) von desktop/electron/listing-text.cjs und android ml/ListingText.kt. Fixture: docs/fixtures/listings/listings.json.
...
export function truncateTitle(text, ellipsis) {
...
export function listingTitle(items) {
...
export function listingDescription(items, priceCents) {
...
export function cardmarketProduct(g) {
  return [nameOf(g), known(g.set_code) ? g.set_code : null].filter((p) => p != null).join(' ');
}
export function pieceCents(totalCents, quantity) {
  return Math.round(totalCents / quantity);
}
export function cardmarketEntry(g, priceCents) {
...
export function afterListingSale(listing, liveCopyIds, soldCopyIds) {
...
export function listingLink(channelId, { cmUrl = null, nameEn = null, setCode = null } = {}) {
...
export function channelShort(channelId, name) {
  if (FIXED_SHORT[channelId]) return FIXED_SHORT[channelId];
  return String(name ?? '').trim().slice(0, 2).toUpperCase() || '??';
}

export function rowTitle(listing, liveItems) {
  if (listing.channel_id === 'cardmarket') {
...
export function listingMarks(listings, items, copyLive, saleStatusOf, suggestionOf) {
...
export function activeByCopy(listings, items) {
  const active = new Map(listings.filter(isActive).map((l) => [l.listing_id, l]));
  const out = {};
  for (const it of items || []) {
...
export function listingsSummary(listings, items) {
...
export function startText(n) {
  return `Angebote: ${n} aktiv`;
}

export function daysSince(listedOn, today) {
